...
void GFX_SetTitle(Bit32s cycles,Bits frameskip,bool paused) {
		SDL_WM_SetCaption(DOSBOXCUSTOM_APPNAME,DOSBOXCUSTOM_VERSION);
}
#else
void GFX_SetTitle(Bit32s cycles,Bits frameskip,bool paused){
	char title[200]={0};
...
Bitu GFX_SetSize(Bitu width,Bitu height,Bitu flags,double scalex,double scaley,GFX_CallBack_t callback) {
	if (sdl.updating)
		GFX_EndUpdate( 0 );
#ifdef DOSBOXCUSTOM_FORCECCUSTOM
	width = DOSBOXCUSTOM_WIDTH;
	height = DOSBOXCUSTOM_HEIGHT;
#endif
...
static void GUI_StartUp(Section * sec) {
   ...
   if(sdl.desktop.want_type==SCREEN_OPENGL){ /* OPENGL is requested */
	...
	printf("Splash begin\n");
//NOTE: custom splash
	sdl.surface=SDL_SetVideoMode(DOSBOXCUSTOM_WIDTH << 1,DOSBOXCUSTOM_HEIGHT << 1,0,0);
...
int main(int argc, char* argv[]) {
	try {
	    ...
	    chdir(path);
		const char *customCommandLine[5];
		customCommandLine[0] = argv[0];
		customCommandLine[1] = DOSBOXCUSTOM_EXECUTABLE;
		customCommandLine[2] = "-conf";
		customCommandLine[3] = DOSBOXCUSTOM_CONFIGFILE;
		customCommandLine[4] = "-exit";
		CommandLine com_line(5, customCommandLine);
#else
